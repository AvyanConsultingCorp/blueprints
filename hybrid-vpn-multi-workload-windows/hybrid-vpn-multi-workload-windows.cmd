...
:::                   vnet_addrss  gateway_addrss subnt_adrss  loc    
CALL :CREATE_HUB_VNET 10.0.0.0/16 10.0.255.240/28 10.0.0.0/17 eastus 

::                       app vnet_addrss  gateway_addrss subnt_adrss  loc    ilb_fe_ip
CALL :CREATE_SPOKE_VNET app1 10.1.0.0/16 10.1.255.240/28 10.1.0.0/17 eastus 10.1.127.254 
CALL :CREATE_SPOKE_VNET app2 10.2.0.0/16 10.2.255.240/28 10.2.0.0/17 eastus 10.2.127.254 
CALL :CREATE_SPOKE_VNET app3 10.3.0.0/16 10.3.255.240/28 10.3.0.0/17 eastus 10.3.127.254 
...
SET TARGET_RESOURCE_GROUP=%APP_NAME%-%ENVIRONMENT%-rg
SET TARGET_LOCAL_GATEWAY=%APP_NAME%-to-on-prem-lgw
...
:: Create a vpn connection to from this vnet to the TARGET_LOCAL_GATEWAY
CALL :CallCLI azure network vpn-connection create --name %VPN_CONNECTION_NAME% ^
  ...
  --lnet-gateway2 %TARGET_LOCAL_GATEWAY% ^
  --lnet-gateway2-group %TARGET_RESOURCE_GROUP% ^
...
:: Set up the names of things using recommended conventions
SET TARGET_RESOURCE_GROUP=%HUB_NAME%-%ENVIRONMENT%-rg
SET TARGET_LOCAL_GATEWAY=%APP_NAME%-to-%HUB_NAME%-lgw
...
:: Create a site-to-site connection to connect to the hub
CALL :CallCLI azure network vpn-connection create --name %VPN_CONNECTION_NAME% ^
  --vnet-gateway1 %VPN_GATEWAY_NAME% --vnet-gateway1-group %RESOURCE_GROUP% ^
  --lnet-gateway2 %TARGET_LOCAL_GATEWAY% ^
  --lnet-gateway2-group %TARGET_RESOURCE_GROUP% ^
